...
@section('content')
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center" id="mainForm" action="{{ route('developer.careers.index') }}"
                            method="GET">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="hidden" id="hiddenCareerDeactivated" name="hiddenCareerDeactivated"
                                    value="0">
                            </div>
                        </form>
                    </div>
                    ...
                    <div
                        ...
                        <a type="button" href="{{ route('developer.careers.create') }}"
                            ...
                            <div id="filterDropdown"
                                ...
                                <ul class="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
                                    <li class="flex items-center">
                                        <input id="CareerDeactivated" name="CareerDeactivated" type="checkbox"
                                            value="1" {{ request('hiddenCareerDeactivated') == 1 ? 'checked' : '' }}
                                            class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                        <label for="CareerDeactivated"
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Coordinador</th>
                                <th scope="col" class="px-4 py-3">Escuela</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($careers->items() as $c)
                                <tr class="border-b dark:border-gray-700">
                                    {{-- Career info --}}
                                    <td class="flex items-center p-4 mr-0 space-x-2 whitespace-nowrap">
                                        <x-badges.careerbadge :type="$c->abbreviation" :color="$c->color" />
                                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                                            <div class="text-base text-wrap font-semibold text-gray-900 dark:text-white">
                                                {{ $c->name }}
                                            </div>
                                            <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                                                {{ $c->school->name }}</div>
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <button id="{{ $c->abbreviation }}-dropdown-button"
                                            data-dropdown-toggle="{{ $c->abbreviation }}-dropdown"
                                        ...
                                        <div id="{{ $c->abbreviation }}-dropdown"
                                            class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                aria-labelledby="{{ $c->abbreviation }}-dropdown-button">
                                                <li>
                                                    <a href="{{ route('developer.careers.show', $c->abbreviation) }}"
                                                        class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Show</a>
                                                </li>
                                                <li>
                                                    <a href="{{ route('developer.careers.edit', $c) }}"
                                                        class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                                </li>
                                            </ul>
                                            @if ($c->trashed())
                                                <div class="py-1">
                                                    <form action="{{ route('developer.careers.restore', $c) }}"
                                                        method="POST">
                                                        @method('PUT')
                                                        @csrf
                                                        <button type="submit"
                                            ...
                                            @else
                                                <div class="py-1">
                                                    <form action="{{ route('developer.careers.destroy', $c) }}"
                                                        method="POST">
                                                        @method('DELETE')
                                                        @csrf
                                                        <button type="submit"
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                    ...
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span
                            class="font-semibold text-gray-900 dark:text-white">{{ $careers->firstItem() }}-{{ $careers->lastItem() }}</span>
                        of
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $careers->total() }}</span>
                    </span>
                    <ul class="inline-flex items-stretch -space-x-px">
                        ...
                        @endif
                        @foreach ($careers->links()->elements['0'] as $label => $path)
                            @if ($label === $careers->currentPage())
                                <li>
    ...
    <script>
        ...
        document.getElementById('mainForm').addEventListener('submit', function() {
            document.getElementById('hiddenCareerDeactivated').value = document.getElementById('CareerDeactivated')
                .checked ? '1' : '0';
        });
    </script>
@endsection
