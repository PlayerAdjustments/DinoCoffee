...
<div class="relative">
    @if ($icon)
        <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none">
            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                fill="currentColor" viewBox="{{ $options['viewbox'] ?? '0 0 20 20' }}">
                {!! $icon !!}
            </svg>
        </div>
    @endif
    <input type="{{ $type }}" id="{{ $name }}"
        ...
        placeholder="{{ $options['placeholder'] }}" value="{{ old($name, $value) }}"
        @if ($options['datepicker']) datepicker datepicker-format="yyyy-mm-dd" @endif
        @isset($options['regex']) pattern="{{ $options['regex'] }}" @endisset
...
@if ($options['helper'])
    <p id="helper-text-explanation-{{ $name }}" class="mt-2 text-[0.57rem] text-gray-500 dark:text-gray-400">
        {!! $options['helper'] !!}</p>
@endif
